...
//    }
    ...
    public KNeighborsClassifier(String jsonStr) {
        this.clf = new Gson().fromJson(jsonStr, Classifier.class);
        this.nTemplates = this.clf.y.length;
        prob_classes = new int[this.clf.nClasses];
    }
    ...
    public double[] getProb() {
        double[] prob_res = new double[this.clf.nClasses];
        int nTotal = 0;

        // calculate total data
        for (int i = 0; i < this.clf.nClasses; i++) {
            nTotal += prob_classes[i];
        }

        // get probability
        for (int i = 0; i < this.clf.nClasses; i++) {
            prob_res[i] = (double)prob_classes[i] / nTotal;
        }
